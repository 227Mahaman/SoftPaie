...
//post: add Module
$app->post('/addModule', function (Request $request, Response $response, $args = []) {//AddModule
    //get db object
    $pdo = new db();
    $sql = "INSERT INTO groupe_action (libelle_groupe, icon_groupe, bloc_menu, ordre_affichage_groupe) VALUES (?,?,?,?)";
    ...
    $data = $pdo->prepare($sql, [$libelle, $icon, $bloc, $ordre]);
    return $response->write(json_encode($data))
    ->withHeader('Content-type', 'application/json')
    ->withStatus(200);
});
...
//get: delete
$app->post('/{table}/{field}/{value}/{field2}/{value2}', function (Request $request, Response $response) {
    $table = $request->getAttribute('table');
    $field = $request->getAttribute('field');
    $value = $request->getAttribute('value');
    $field2 = $request->getAttribute('field2');
    $value2 = $request->getAttribute('value2');
    $pdo = new db();
    $data = $pdo->query("DELETE FROM $table WHERE $field='$value' AND $field2='$value2'");
    //$data = $pdo->prepare($sql, [$table, $field, $value, $field2, $value2]);
    return $response->write(json_encode($data))
    ->withHeader('Content-type', 'application/json')
    ->withStatus(200);
